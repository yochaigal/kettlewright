...
PATH = '/users/printer/characters/hero/print/landscape.pdf'
...
def test_pdf_route_embeds_fonts_and_builtin_portrait_without_mutating_character(pdf_character):
    app, character = pdf_character
    original = (character.name, character.items, character.containers, character.hp)
    response, reader = rendered(app)
    assert len(reader.pages) == 1
    assert 'Лейф' in contents(reader)
    assert 'inline;' in response.headers['Content-Disposition']
    assert 'hero-landscape-a4.pdf' in response.headers['Content-Disposition']
    ...
    assert app.test_client().get(PATH.replace('/hero/', '/missing/')).status_code == 404


def test_pdf_leaves_current_max_gold_and_armor_blank_for_handwriting(pdf_character):
    app, character = pdf_character
    character.gold = 31415
    character.items = json.dumps([dict(name='Leather armor', tags=['2 Armor'], location=0)])
    db.session.commit()
    _, reader = rendered(app)
...
def test_long_sections_paginate_and_keep_unicode_and_html_text(pdf_character):
    app, character = pdf_character
    ...
    for page in reader.pages:
        assert float(page.mediabox.width) == pytest.approx(841.89)
        assert float(page.mediabox.height) == pytest.approx(595.276)


def test_inventory_retains_overflow_petty_bulky_and_additional_containers(pdf_character):
    app, character = pdf_character
...
def test_portrait_upload_is_embedded_and_external_url_is_preserved(pdf_character, tmp_path):
    app, character = pdf_character
...
def test_missing_portrait_and_extreme_unbroken_text_do_not_break_pdf(pdf_character):
    app, character = pdf_character
    character.custom_image = True
...
def test_character_view_keeps_html_print_and_adds_pdf_option(pdf_character):
